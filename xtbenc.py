...
layout = [[sg.Text('Input', size=(7,1)), sg.InputText(key='_infile_', size=(116,1)), sg.FileBrowse(size=(8,1))],
		  ...
		  [sg.Frame('LOG', font='Any 15', layout= [[sg.Output(size=(130, 30))]])],
		  
		  [sg.Button('Convert'), sg.SimpleButton('Exit', button_color=('white','firebrick3'))]]
...
while True:
	...
	video_input = values['_infile_']
	video_output = values['_outfile_']
	myargs = values['_editor_']


	ffp2 = ['ffprobe', '-hide_banner', video_input]
	ffp5 = ['ffprobe', '-hide_banner', video_output]

	cmd1 = [] # Radio button selection

	if values['_CPU'] == True: cmd1 = "ffmpeg " "-y " "-i " + "'%s'"%video_input + " " + myargs + " -v " "verbose " + "'%s'"%video_output
	
	if values['_QSV'] == True: cmd1 = "ffmpeg " "-y " "-i " + "'%s'"%video_input + " " + myargs + " -v " "verbose " + "'%s'"%video_output
	
	if values['_VAAPI'] == True: cmd1 = "ffmpeg " "-y " "-vaapi_device " ":0 " "-i " + "'%s'"%video_input + " " + myargs + " -v " "verbose " + "'%s'"%video_output
	
	if values['_NVENC'] == True: cmd1 = "ffmpeg " "-y " "-i " + "'%s'"%video_input + " " + myargs + " -v " "verbose " + "'%s'"%video_output
	...
	def invoke_process_popen_poll_live(command):
		process = subprocess.Popen(command, stdin=subprocess.DEVNULL, stdout=subprocess.DEVNULL, stderr=subprocess.PIPE, universal_newlines=True)
